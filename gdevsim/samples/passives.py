...
@gf.cell
def straight_wire(width: float = 10,
                    pad_length: float = 2,
                    metal_length: float = 10,
                    pad_layer: LayerSpec = LAYER.PAD,
                    metal_layer: LayerSpec = LAYER.MTOP,
                ) -> gf.Component:
    """Wire."""
    c = gf.Component()
    left_pad = c.add_ref(gf.components.rectangle(size=(pad_length, width), layer=pad_layer))
    right_pad = c.add_ref(gf.components.rectangle(size=(pad_length, width), layer=pad_layer))
    wire = c.add_ref(gf.components.rectangle(size=(metal_length + 2*pad_length, width), layer=metal_layer))
    wire.xmin = left_pad.xmin
    right_pad.xmax = wire.xmax
    c.add_port("e1", port=left_pad.ports["e1"])
    c.add_port("e2", port=right_pad.ports["e3"])
    return c
...
def parallel_plate_capacitor(width: float = 10,
    ...
    c = gf.Component()
    plate1 = c.add_ref(gf.components.rectangle(size=(length, width), layer=layer1))
    plate2 = c.add_ref(gf.components.rectangle(size=(length, width), layer=layer2))
    c.add_port("e1", port=plate1.ports["e1"])
    c.add_port("e2", port=plate2.ports["e2"])
    return c
...
def mimcap(
    ...
    Adapted from https://github.com/gdsfactory/skywater130/blob/main/sky130/pcells/mimcap_1.py
...
if __name__ == "__main__":
    c = mimcap(m4_length=5, m4_width=5, m4_r_length=1)
